...
function setupPopup() {
  ...
  downloadBtn.onclick = () => {
    browserAPI.runtime.sendMessage({
      action: ACTIONS.DOWNLOAD_IMGS,
      imgUrls: [...selectedImgs],
    });
  };
  ...
  selectAllCheck.onchange = (event) => {
    ...
    const imgCardElements = Array.from(
      imgContainer.getElementsByClassName("img-card")
    );
  ...
  optionsTitle.textContent = i18n.OPTIONS;
  optionsBtn.onclick = () => {
...
function renderImages(imgUrls) {
  if (imgUrls.length === 0) {
    ...
    disableImageActions();
    return; // Stop rendering if no images are found 
  }
  ...
  imgUrls.forEach((imgUrl) => {
    ...
    let imgFileName = imgUrl.match("[^/]*$")[0];
    let truncName = imgFileName.replace(/^(.{20}).*/, "$1..."); // cut off name after 19 chars
    imgFileName = imgFileName.length > 20 ? truncName : imgFileName;
    imgName.textContent = imgFileName;
    imgName.className = "img-name";

    imgCard.appendChild(imgSelectCheck);
    imgCard.appendChild(imgLinkBtn);
    imgCard.appendChild(imgElement);
    imgCard.appendChild(imgName);
...
runInCurrenTab(async (tab) => {
  browserAPI.tabs.sendMessage(
    tab.id,
    { action: ACTIONS.GET_ALL_IMG_URLS },
    (res) => {
